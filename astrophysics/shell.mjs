#!/usr/bin/env node
/* Render the Astrophysics microsite's page shell from university-web-patterns.
 *
 *     node astrophysics/shell.mjs           # rewrite the five pages in place
 *     node astrophysics/shell.mjs --check   # exit 1 if any page is out of date
 *     node astrophysics/shell.mjs --ids     # the component ids the bundle needs
 *
 * Modelled on quantum/shell.mjs, which is the worked example. Read that file's
 * header first; this one only records where Astrophysics differs.
 *
 ...
 *         --ids "$(node astrophysics/shell.mjs --ids)" \
 *         > astrophysics/patterns.templates.js
 *
  ...
  if (missing.length) {
    throw new Error(
      `${SHARED_RENDERER} renders ${missing.join(", ")}, which is not in ` +
        "TEMPLATE_IDS in astrophysics/shell.mjs. Add it, re-emit " +
        "astrophysics/patterns.templates.js, and re-run this script.",
    );
  }
}
...
function checkStylesheets() {
  ...
  if (missing.length) {
    throw new Error(
      `astrophysics/style.css does not import ${missing.join(", ")}, which ` +
...
const site = {
  name: "Astrophysics Research Group",
  parent_name: "Department of Physics",
  home_url: "index.html",
};
...
const breadcrumbAncestors = [
  ...
  { label: "Astrophysics group", href: "index.html" },
];
...
const NAV_ID = "astrophysics-primary-nav";
  ...
  if (missing.length) {
    throw new Error(
      `${file}: the rendered shell lost legacy hook(s) ${missing.join(", ")}. ` +
        "Update LEGACY_DECORATIONS in astrophysics/shell.mjs to match the " +
...
function main(argv) {
  ...
  if (check && stale.length) {
    process.stderr.write(
      `astrophysics shell is out of date in: ${stale.join(", ")}\n` +
        "Run: node astrophysics/shell.mjs\n",
    );
    return 1;
  }
  ...
  process.stdout.write(
    check
      ? `astrophysics shell matches university-web-patterns ${version}\n`
      : `astrophysics shell rendered into ${pages.length} pages ` +
